...
Cette matrice documente les mécanismes de protection actuels et dans le pipeline CI à l'issue de **LOT-003** (Security & ESI Hardening).
...
| **Security Hardening (HTTP & Auth)** | `npm run test:security` (`server/__tests__/security_hardening.test.ts`) | Étape `Security hardening tests` dans `.github/workflows/ci.yml` | ✅ COUVERT | 15 tests : validation des entrées (types, limits, bounds, integer), en-têtes de sécurité (nosniff, X-XSS-Protection, Referrer-Policy), masquage `x-powered-by`, CORS preflight & credentials, unicité du jeton CSRF OAuth (anti-rejeu), assainissement XSS HTML callback, rejet des payloads volumineux. |
