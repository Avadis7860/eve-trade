# ADR-0003 — Economic Position and Market-Order Model
...
Character and corporation are not separate automatic accounting entity types.
...
A disposal creates one or more `DisposalAllocation` records under deterministic FIFO.
...
Capital recovery measures cash recovered against capital committed by the position:
...
Capital recovery may be positive while a position is open, but that does not authorize a whole-position profitable classification.
...
Missing coverage is UNKNOWN/PARTIAL/UNAVAILABLE, not zero.

`UNKNOWN`, `PARTIAL`, `ERROR`, `ABSENT`, `UNAVAILABLE` and `STALE` must not be converted into synthetic numeric facts.

### 10. Fees do not control lifecycle

Fee evidence is independent from physical/economic closure.

When fee-inclusive net result cannot be established, the fee breakdown and net ratios remain unavailable; lifecycle may still reach `CLOSED`.
...
Portfolio/UI and broad TraderAnalytics migration remain downstream work and are not part of this recovery.
