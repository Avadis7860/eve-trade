...
Deep review of the active branch now establishes the ecosystem-level accounting boundary:
...
4. Characters and corporations are participants inside the same trading/industrial ecosystem by default. They are attribution dimensions, not automatic accounting silos. A purchase by A can legitimately fund a later sale by B when both transactions belong to the same ecosystem.

Market observation, economic transaction, position lifecycle, realized financial state, position-level capital recovery and ecosystem attribution must therefore remain distinct.
...
- CI regressions are corrected and the branch is green on CI #923 for the current intermediate increment; semantic certification remains blocked until ecosystem-level matching is implemented and tested.
