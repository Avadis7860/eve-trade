...
| Agent context | CLOSING ON DELIVERY BRANCH / STABLE AFTER MERGE | .eve-trade/context-map.json + current-work + context-integrity + ci-scope | test:context + CI static + Main Smoke stable check | [agent context](../operations/agent-context.md) | lifecycle/bootstrap/functional-routing + raw-commit stable-anchor hardening |
