...
function run() {
...

  {
    ...
    assert(p.capital_recovery_state === 'NEGATIVE', 'capital recovery state must remain negative');
  }
...

  {
    ...
    assert(state.position_recovery_delta === 20, '7,143 sales recover 1,000,020 ISK against 1,000,000 capital');
    assert(state.position_recovery_state === 'POSITIVE', 'capital recovery can be positive while position remains partially realized');
  }
...

  {
    ...
    assert(result.position.position_completeness === 'PARTIAL', 'causal inventory deficit must be PARTIAL');
...

  {
    ...
    assert(result.position.quantity_acquired === null, 'unprovenanced acquisition must not fabricate an economic quantity');
  }
