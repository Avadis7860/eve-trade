...
function positionRecoveryState(delta: number): import('../types').EconomicPositionRecoveryState {
...
export function reconstructPositionLedger(
  ...
  const activeSegmentLotIds = new Set<string>();
  let segmentSequence = 0;
  let activeSegmentId: string | null = null;
  let segmentQuantityAcquired = 0;
  let segmentCapitalCommitted = 0;
  let segmentCashRecovered = 0;

  for (const tx of valid) {
    const timestamp = transactionTimestamp(tx)!;

    if (tx.is_buy) {
      if (activeSegmentLotIds.size === 0) {
        segmentSequence += 1;
        activeSegmentId = ['position', accountingScopeId, typeId, segmentSequence].join('_');
        segmentQuantityAcquired = 0;
        segmentCapitalCommitted = 0;
        segmentCashRecovered = 0;
      }

      const originalCost = roundIsk(tx.quantity * tx.unit_price);
      const lot: AcquisitionLot = {
        lot_id: 'acquisition_' + tx.transaction_id,
        position_segment_id: activeSegmentId!,
      ...
      lots.push(lot);
      activeSegmentLotIds.add(lot.lot_id);
      segmentQuantityAcquired += tx.quantity;
      segmentCapitalCommitted = roundIsk(segmentCapitalCommitted + originalCost);
      continue;
    }

    const segmentIdAtDisposition = activeSegmentId;
    let remainingSellQuantity = tx.quantity;

    if (segmentIdAtDisposition && activeSegmentLotIds.size > 0) {
      for (let index = 0; index < lots.length && remainingSellQuantity > 0; index += 1) {
        const lot = lots[index];
        if (!activeSegmentLotIds.has(lot.lot_id) || lot.remaining_quantity <= 0) continue;
        if (lot.location_id !== tx.location_id) {
          unreconciledLocationTransitionCount += 1;
          unreconciledLocationTransitionsBySegment.set(
            segmentIdAtDisposition!,
            (unreconciledLocationTransitionsBySegment.get(segmentIdAtDisposition!) ?? 0) + 1,
          );
        }
        ...
        allocations.push({
          ...
          position_segment_id: segmentIdAtDisposition!,
        ...
        segmentCashRecovered = roundIsk(segmentCashRecovered + revenue);
      }
    }
    ...
    const remainingPositionQuantity =
      activeSegmentLotIds.size > 0
        ? lots.filter((lot) => activeSegmentLotIds.has(lot.lot_id))
    ...
    if (segmentIdAtDisposition) {
      const recoveryDelta = roundIsk(segmentCashRecovered - segmentCapitalCommitted);
      const recoveryRatio =
        segmentCapitalCommitted > 0 ? segmentCashRecovered / segmentCapitalCommitted : null;
      dispositionStates.push({
        ...
        position_quantity_acquired: segmentQuantityAcquired,
        position_capital_committed: segmentCapitalCommitted,
        position_cash_recovered: segmentCashRecovered,
    ...
    if (activeSegmentLotIds.size > 0 && remainingPositionQuantity === 0) {
      activeSegmentLotIds.clear();
  ...
  const buildPositionSegment = (positionSegmentId: string): CurrentPosition => {
    ...
    return Object.freeze({
      ...
      capital_recovery_state:
        segmentRecoveryDelta === null ? null : positionRecoveryState(segmentRecoveryDelta),
