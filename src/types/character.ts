...
export interface EveCharacterTransaction {
  ...
  /** Explicit economic accounting scope; character identity is attribution, not scope. */
  accounting_scope_id?: string;
  /** Source/provenance preserved when a transaction crosses the financial boundary. */
  provenance?: import('./financial').FinancialProvenance;
  economic_owner_type?: import('./financial').EconomicOwnerType;
...
export interface TradeCycleRecord {
  ...
  hold_days: number;
  /** Null when profitability cannot be established from available net evidence. */
...
export interface TraderPerformanceMetrics {
  ...
  /** Null when at least one included disposal result has unavailable net evidence. */
